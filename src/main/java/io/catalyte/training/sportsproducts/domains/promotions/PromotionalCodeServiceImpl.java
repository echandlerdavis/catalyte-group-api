...
public class PromotionalCodeServiceImpl implements PromotionalCodeService {
   ...
   */
  @Override
  public PromotionalCode createPromotionalCode(PromotionalCodeDTO promotionalCodeDTO)
    ...
    PromotionalCode existingTitle = promotionalCodeRepository.findByTitle(
        promotionalCode.getTitle());
    if (existingTitle != null) {
      throw new IllegalArgumentException(
          String.format("Title %s is already in use", existingTitle.getTitle()));
    }
    ...
    try {
      ...
      if (promotionalCode.getType() == PromotionalCodeType.FLAT
          && promotionalCode.getRate().compareTo(BigDecimal.ZERO) <= 0) {
        throw new IllegalArgumentException("Flat promotional code rate must be greater than zero");
      }

      if (promotionalCode.getType() == PromotionalCodeType.PERCENT && (
          promotionalCode.getRate().compareTo(BigDecimal.ZERO) <= 0
              || promotionalCode.getRate().compareTo(BigDecimal.valueOf(100)) > 0)) {
        throw new IllegalArgumentException(
            "Percentage promotional code rate must be greater than zero and less than or equal to 100");
      }
   ...
   * @throws IllegalArgumentException if the given promotional code is invalid.
   */
  private void validatePromotionalCode(PromotionalCode promotionalCode)
      throws IllegalArgumentException {
    try {
      if (promotionalCode.getType() == null || promotionalCode.getRate() == null) {
      ...
      if (promotionalCode.getType() == PromotionalCodeType.FLAT
          && promotionalCode.getRate().compareTo(BigDecimal.ZERO) <= 0) {
        throw new IllegalArgumentException(
            "Invalid promotional code: flat rate must be greater than zero");
      }
      ...
      if (promotionalCode.getType() == PromotionalCodeType.PERCENT && (
          promotionalCode.getRate().compareTo(BigDecimal.ZERO) <= 0
              || promotionalCode.getRate().compareTo(BigDecimal.valueOf(100)) > 0)) {
        throw new IllegalArgumentException(
            "Invalid promotional code: percent rate must be between 0 and 100");
      }

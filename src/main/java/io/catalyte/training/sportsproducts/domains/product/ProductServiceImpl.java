...
import static io.catalyte.training.sportsproducts.domains.product.ProductFilterTypes.valueOf;
...
public class ProductServiceImpl implements ProductService {
  ...
  public List<Product> getProductsByCategories(List<Product> products, String categories) {
    ...
    Arrays.asList(categories.split("\\|"))
        .forEach(category -> categoryNames.add(category.toLowerCase()));
   ...
   *
   * @param products     the list of products to filter
   * @param demographics the list of demographics to filter by as a string with | between each
   *                     value
   * @return filtered list of products
   */
  public List<Product> getProductsByDemographics(List<Product> products, String demographics) {
    ...
    Arrays.asList(demographics.split("\\|"))
        .forEach(demographic -> demographicNames.add(demographic.toLowerCase()));

    products.removeIf(
        product -> !demographicNames.contains(product.getDemographic().toLowerCase()));
   ...
   * @return filtered list of products
   */
  public List<Product> getProductsByPrice(List<Product> products, String priceMin,
      String priceMax) {
   ...
   *
   * @param products      the list of products to filter
   * @param primaryColors the list of primaryColors to filter by as a string with | between each
   *                      value
   * @return filtered list of products
   */
  public List<Product> getProductsByPrimaryColors(List<Product> products, String primaryColors) {
    ...
    Arrays.asList(primaryColors.split("\\|"))
        .forEach(primaryColor -> primaryColorsCodes.add(primaryColor.toLowerCase()));

    products.removeIf(
        product -> !primaryColorsCodes.contains(product.getPrimaryColorCode().toLowerCase()));
  ...
  public List<Product> getProductsByMaterials(List<Product> products, String materials) {
    ...
    Arrays.asList(materials.split("\\|"))
        .forEach(material -> materialNames.add(material.toLowerCase()));
